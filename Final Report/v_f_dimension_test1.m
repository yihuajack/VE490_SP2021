...
%%% design parameters %%%
%%% test 1 %%%
L = [50e-6,25e-6,10e-6,5e-6];  % large electrode length
S = [30e-6,15e-6,6e-6,3e-6]; % small electrode length
G = [20e-6,10e-6,4e-6,2e-6]; % inter electrode gap
...
for i = 1:4
    ...
    lel = L(i)*larray/lpair; % only the L electrode vgenerates forward velocity
    gamma = lel/ltot; % ratio electrode along the channel to channel length
    sidesElectr = 1; % sides with electrodes
    beta = sidesElectr/4; 
    
    % factor for average velocity above electrodes
    %%% derived parameters %%%
    k = L(i)/S(i);
    xmax = (G(i)+L(i)+S(i)) /( sqrt(k) + 1/sqrt(k))
    %normalised electrode end coordinate.
    ...
    semilogx(f,(vLave*beta*gamma)/10^(-6)) 
    % plot max velocity of Poiseuille flow
    hold on;
end
...
legend({'50-30-20\mum','25-15-10\mum','10-6-4\mum','5-3-2\mum'},'Location','northwest')
text(20,115,'c_0 = 0.01mM','FontSize',9);
xlim([10 100000])
xlabel('Frequency / Hz')
